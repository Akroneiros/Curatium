...
class Encrypt extends CNG
{
	...
	static File(AlgId, Mode, FileNameIn, FileNameOut, Key, IV := "", Bytes := 1048576, Offset := 0, Length := -1, Encoding := "UTF-8")
	{
		...
		try
		{
			...
			FileOut.Seek(0)
			FileIn.Pos := Offset ; ** ADDED (2025-08-12): Include Byte Order Mark (BOM) when Offset=0 to avoid identical hashes for files with and without BOM. **
...
class Hash extends CNG
{
	...
	static File(AlgId, FileName, Bytes := 1048576, Offset := 0, Length := -1, Encoding := "UTF-8", Output := "HEXRAW")
	{
		...
		try
		{
			...
			File.Pos := Offset   ; ** ADDED (2025-08-12): Include Byte Order Mark (BOM) when Offset=0 to avoid identical hashes for files with and without BOM. **
